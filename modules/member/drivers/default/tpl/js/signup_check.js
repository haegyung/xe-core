...
function memberSetEvent(fieldList) {
	var filterArr = new Array();
	var i;
	for(i = 0; i < fieldList.length; i++)
	{
		var fieldName = fieldList[i];
		filterArr.push('[name=' + fieldName + ']');
	}
...
function memberCheckValue(event) {
	...
	var _driver = jQuery('[name=driver]').val();
	var _member_srl = current_url.getQuery('member_srl');
	var module = current_url.getQuery('module');
	...
	var params = {name:_name, value:_value, driver:_driver, member_srl:_member_srl};
